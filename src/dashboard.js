...
function setupTabs() {
  ...
  function activateTab(selected) {
    ...
    if (selected === "charts") {
      ...
      // Initialize network only when visible.
...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    // Setup tab switching; the tracker network graph will initialize on demand.
    setupTabs();
  } catch (err) {
...
function initBarChart(results) {
  const validResults = (Array.isArray(results) ? results : []).filter(
    (r) => r && typeof r.domain === "string"
  );
  const chart = echarts.init(document.getElementById("bar-chart"));
  const domains = validResults.map((r) => r.domain);
  const trackerCounts = validResults.map((r) =>
    Array.isArray(r.trackers) ? r.trackers.length : 0
  );
  chart.setOption({
...
function initPieChart(analytics) {
  ...
  const categories = Array.isArray(analytics.topTrackerCategories)
    ? analytics.topTrackerCategories
    : [];
...
function initStackedBarChart(analytics) {
  ...
  Object.entries(byCountry).forEach(([country, data]) => {
    data.topTrackerOwners.forEach((owner) => {
      const ownerName =
        owner.name?.displayName || owner.name?.name || "Unknown";
      countryData[country] = countryData[country] || {};
      countryData[country][ownerName] =
        (countryData[country][ownerName] || 0) + owner.count;
    });
  });
  const countries = Object.keys(countryData);
  const owners = Array.from(
    new Set(countries.flatMap((c) => Object.keys(countryData[c])))
  );
...
function renderDomainDetails(siteData) {
  ...
  infoDiv.innerHTML = `
    ...
    <p><strong>Page Size:</strong> ${siteData.totalSize.toLocaleString()} bytes</p>
...
function initTrackerNetwork(results) {
  // Build nodes and links for a force-directed graph.
  const siteNodes = [];
  const trackerNodes = {};
  const links = [];
  ...
  chart.setOption({
    ...
    legend: { data: ["Sites", "Trackers"], bottom: 10 },
    series: [
      {
        type: "graph",
        layout: "force",
