var $2Xvvg$echarts = require("echarts");


let $588d3bf34bb3bc0f$var$globalResults = [];
function $588d3bf34bb3bc0f$var$setupTabs() {
    const tabs = {
    ...
    function activateTab(selected) {
        ...
        if (selected === "charts") {
            ...
            if (!trackerNetworkInitialized) setTimeout(()=>{
                $588d3bf34bb3bc0f$var$initTrackerNetwork($588d3bf34bb3bc0f$var$globalResults);
...
document.addEventListener("DOMContentLoaded", async ()=>{
    try {
        ...
        $588d3bf34bb3bc0f$var$globalResults = results;
        $588d3bf34bb3bc0f$var$initBarChart(results);
        $588d3bf34bb3bc0f$var$initPieChart(analytics);
        $588d3bf34bb3bc0f$var$initStackedBarChart(analytics);
        $588d3bf34bb3bc0f$var$initDoughnutChart(analytics);
        $588d3bf34bb3bc0f$var$initDomainLookup(results);
        $588d3bf34bb3bc0f$var$setupTabs();
    } catch (err) {
        console.error("Dashboard data error:", err);
        alert("Failed to load dashboard data.");
    }
});
function $588d3bf34bb3bc0f$var$initBarChart(results) {
    const chart = $2Xvvg$echarts.init(document.getElementById("bar-chart"));
...
function $588d3bf34bb3bc0f$var$initPieChart(analytics) {
    const chart = $2Xvvg$echarts.init(document.getElementById("pie-chart"));
...
function $588d3bf34bb3bc0f$var$initStackedBarChart(analytics) {
    ...
    const chart = $2Xvvg$echarts.init(document.getElementById("stacked-bar-chart"));
    chart.setOption({
        tooltip: {
            trigger: "axis",
            axisPointer: {
...
function $588d3bf34bb3bc0f$var$initDoughnutChart(analytics) {
    const domainCounts = {};
    ...
    const top = Object.entries(domainCounts).sort((a, b)=>b[1] - a[1]).slice(0, 10).map(([name, value])=>({
            name: name,
            value: value
        }));
    const chart = $2Xvvg$echarts.init(document.getElementById("doughnut-chart"));
    chart.setOption({
        tooltip: {
            trigger: "item"
        },
...
function $588d3bf34bb3bc0f$var$initDomainLookup(results) {
    ...
    select.addEventListener("change", ()=>{
        ...
        $588d3bf34bb3bc0f$var$renderDomainDetails(site);
    });
...
function $588d3bf34bb3bc0f$var$renderDomainDetails(site) {
  ...
  `;
    const tableRows = (site.trackers || []).map((t)=>`
      <tr>
...
function $588d3bf34bb3bc0f$var$initTrackerNetwork(results) {
    const siteNodes = [];
    const trackerNodes = {};
    const links = [];
    results.forEach((site)=>{
        ...
        (site.trackers || []).forEach((tracker)=>{
            const domain = $588d3bf34bb3bc0f$var$normalizeDomain(tracker.domain);
            const trackerId = `tracker-${domain}`;
            if (!trackerNodes[trackerId]) trackerNodes[trackerId] = {
                id: trackerId,
                name: domain,
                category: 1,
                symbolSize: 35
    ...
    const nodes = [
        ...siteNodes,
        ...Object.values(trackerNodes)
    ];
    const chart = $2Xvvg$echarts.init(document.getElementById("tracker-network-chart"));
    chart.setOption({
        tooltip: {
...
function $588d3bf34bb3bc0f$var$normalizeDomain(domain) {
    try {
